...
async function checkPatClassifications() {
  try {
    ...
    if (patClassifications.length > 0) {
      console.log(`   First classification: ${patClassifications[0].created_at}`);
      console.log(`   Last classification: ${patClassifications[patClassifications.length - 1].created_at}`);
      console.log(`   Sample image IDs: ${patClassifications.slice(0, 5).map(c => c.image_id).join(', ')}`);
    }
    ...
    // Check for any recent classifications
    const today = new Date().toISOString().split('T')[0];
    const recentClassifications = patClassifications.filter(c => 
      c.created_at && c.created_at.startsWith(today)
    );
    
    if (recentClassifications.length > 0) {
      console.log(`\n📅 Recent Classifications (today):`);
      console.log(`   Count: ${recentClassifications.length}`);
      recentClassifications.forEach(c => {
        console.log(`     - ${c.image_id} at ${c.created_at}`);
      });
    }
