...
class NIFlags(DatasetBase):
    ...
    def read_expert_annotations(self):
        """
        Read your 8,204 expert classifications and convert to hierarchical format
        """
        ...
        for image_name, annotation in annotations.items():
            ...
            # Create hierarchical classname in format: "category-context-specific_flag"
            category = annotation['category'].replace(' ', '_')
            context = annotation['context'].replace(' ', '_').replace('-', '_')
            specific_flag = annotation['specific_flag'].replace(' ', '_')
            
            # Create hierarchical classname (this is what Li et al.'s code expects)
